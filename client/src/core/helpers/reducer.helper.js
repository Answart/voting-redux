...
export function getUpdatedList(list, item, filters = null) {
  ...
  if (!!filters && filters.length > 0) {
    const itemPassesFilter = filters.every(filter => item[`${filter.key}`] === filter.value);
...
export function getFilteredList(list = null, filters = null) {
  ...
  const filteredList = list.filter(item => filters.every(filter => item[`${filter.key}`] === filter.value));
  return filteredList;
};
